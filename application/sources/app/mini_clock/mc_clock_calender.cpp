...
static void mc_calendar_field_change(int8_t delta)
{
    ...
    case MC_CAL_FIELD_YEAR:
        if (delta > 0)
        {
            if (ev->year < 2099) ev->year++;
        }
        else
        {
            if (ev->year > 2000) ev->year--;
        }
        /* Re-clamp day */
        days = mc_calendar_days_in_month(ev->month, ev->year);
        if (ev->day > days) ev->day = days;
        break;
...
void mc_clock_calendar_handle(ak_msg_t* msg)
{
    ...
    case MC_CLOCK_CALENDAR_DELETE_EVENT:
    {
        ...
        /* When cancelling a new event from EDIT mode, use editing_event index */
        uint8_t del = (calendar.mode == MC_CAL_MODE_EDIT)
                          ? calendar.editing_event
