...
static void view_scr_calendar_list(const mc_calendar_state_t* st)
{
    ...
    view_render.drawLine(0, 57, 128, 57, WHITE);
    view_render.setCursor(2, 59);
    view_render.print("MD:edit L-MD:del BK:grid");
}
...
static const char* const FIELD_LABEL[MC_CAL_FIELD_MAX] = {
    ...
    "Start H", "Start M", "End H", "End M", "Alarm",
};
...
static void view_scr_calendar_edit(const mc_calendar_state_t* st)
{
    ...
    case MC_CAL_FIELD_END_HOUR:
        xsprintf(buf, "%02u", (st->editing_field == MC_CAL_FIELD_START_HOUR)
                                  ? ev->start_hour : ev->end_hour);
        break;
    case MC_CAL_FIELD_START_MIN:
    case MC_CAL_FIELD_END_MIN:
        xsprintf(buf, "%02u", (st->editing_field == MC_CAL_FIELD_START_MIN)
                                  ? ev->start_min : ev->end_min);
        break;
    ...
    /* Progress bar */
    uint8_t bar_w = (uint8_t)(((uint16_t)(st->editing_field + 1) * 110)
                               / MC_CAL_FIELD_MAX);
...
void scr_clock_calender_handle(ak_msg_t* msg)
{
    ...
    case AC_DISPLAY_BUTON_LONG_MODE_PRESSED:
    {
        ...
        case MC_CAL_MODE_EDIT:
            /* Long-press MODE in edit view = go back without saving */
            mc_clock_calendar_get_state(&st);
            if (st.is_new_event)
            {
                /* Point current_event at the new (unconfirmed) entry so delete works */
                /* The calendar task will delete the event at editing_event index */
                task_post_pure_msg(MC_CLOCK_CALENDAR_ID, MC_CLOCK_CALENDAR_DELETE_EVENT);
            }
    ...
    /* ------------------------------------------------------------------ */
    /* Return to menu — only from month view via a dedicated exit gesture  */
    /* (Not mapped to a button yet; menu handles back navigation globally) */
    default:
        break;
